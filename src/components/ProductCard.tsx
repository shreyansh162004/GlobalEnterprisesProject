...
const ProductCard = ({ product, index = 0 }: ProductCardProps) => {
  ...
  const handleShare = async (e: React.MouseEvent) => {
    ...
    const text = `Check out ${product.name} - ₹${product.price.toLocaleString("en-IN")} at Global Enterprises`;
    if (navigator.share) {
  ...
  return (
    ...
    >
      <div className="block group overflow-hidden rounded-2xl transition-all duration-300 hover:-translate-y-2 glass-card hover:shadow-2xl hover:shadow-primary/15">
        ...
        <div className="p-5 space-y-3">
          <div>
            ...
            <Link to={`/products/${product.id}`} className="font-heading font-bold text-base mt-0.5 leading-tight block hover:underline">
              {product.name}
            </Link>
          </div>
          <p className="text-xs line-clamp-1 text-muted-foreground">{product.specs}</p>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              onClick={handleShare}
              className="flex items-center justify-center px-3 py-2.5 rounded-xl bg-secondary/50 border border-border text-xs font-bold transition-all hover:bg-secondary"
            >
